...
main = do
    ...
    case maybeInput of 
        ...
        Just (SelectRequest bullseye file) -> do
            ...
            let sf = simFunc func file value
            getTuples file value sf nameOfFile
            putStrLn $ "Successfully completed"
        Just (OpenRequest file condition) -> do
            let cond = parseCondition condition
            let filteredProteins = filterProteins file condition
...
parseCondition s = case s of
    ...
    _ -> Nothing
    where
        readValue = readMaybe::String->maybeInt
        lessThan x = (< x)
        greatherThan x = (> x)
        equalTo x = (== x)
...
-- parseInput :: String -> Maybe(String, String, String)
-- parseInput input  = case words input of
--     ["Select", bullseye, "from", file, "where", condition ] -> Just(bullseye, file, condition)
--     ["From", file, "show", condition]->Just(file,condition,"")
--     _->Nothing

parseInput :: String -> Maybe Request
parseInput input = case words input of
    ["Select", bullseye, "from", file]->Just $SelectRequest bullseye file
    ["Open", file, "and show", condition] -> Just $OpenRequest file condition
